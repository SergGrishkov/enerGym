import{E,f as x,c as R,r as y}from"./assets/pagination-0e1a19b4.js";import"./assets/vendor-6fe7236c.js";document.addEventListener("DOMContentLoaded",function(){const e=document.querySelector(".footer-form"),s=document.getElementById("confirmationModal"),t=document.getElementById("modalSubscribeText");let n=new E,o;function U(r){const c=document.createElement("div");c.classList.add("error-message"),c.textContent=r,e.parentNode.insertBefore(c,e.nextSibling),setTimeout(function(){c.remove()},2e3)}e.addEventListener("submit",async function(r){r.preventDefault();const c=e.querySelector("input[name='footer-email']").value;if(!/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/.test(c)){U("Email format is incorrect");return}try{const O=(await n.createSubscription({email:c})).info();t.textContent=O.message,s.classList.add("is-open"),e.querySelector("input[name='footer-email']").value="",o=setTimeout(function(){s.classList.remove("is-open")},8e3)}catch($){console.error("Error:",$)}}),document.querySelectorAll(".modal-subscribe-close-btn, .modal-subscribe-close-button").forEach(function(r){r.addEventListener("click",function(){s.classList.remove("is-open"),clearTimeout(o)})}),document.addEventListener("keydown",function(r){r.key==="Escape"&&s.classList.contains("is-open")&&(s.classList.remove("is-open"),clearTimeout(o))}),window.addEventListener("click",function(r){r.target===s&&(s.classList.remove("is-open"),clearTimeout(o))})});const u=document.getElementById("cards-list"),_=document.querySelector(".pagination-list"),h=document.querySelector(".exercsise-pagination-list"),S=new E,m=document.querySelector(".slash"),f=document.querySelector(".home-filters-subtitle"),L=document.querySelector(".form"),q=window.innerWidth;let z=1,J,p="",w;const i={page:z,limit:J},a={filter:"",name:""};function B(e){return e<1280?i.limit=8:i.limit=9,i.limit}function V(e,s){a.filter=e,a.name=s.toLowerCase()}function A(e,s,t){C(),i.page=t,s==="formSearch"?Z(e,i.page):P(a.filter,a.name)}async function P(e,s){C(),B(q);const t={[e]:s,...i};try{const n=await(await S.getListExercisesBySubspecies(t)).json();if(n.results){const o=n.results;m.classList.add("opacity-animating"),f.classList.add("opacity-animating"),m.textContent="/",u.innerHTML=H(o),v(),I.insertAdjacentElement("beforeEnd",L),_.innerHTML="",h.innerHTML=y(n.totalPages,i.page)}}catch(n){console.log(n)}}function H(e){return e.reduce((s,t)=>s+`<li class="list-item animated-card" data-exerciseid="${t._id}">
