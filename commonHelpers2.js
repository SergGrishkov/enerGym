import{E as M,f as A}from"./assets/quote-a8d88175.js";import"./assets/vendor-0fffa566.js";const h=document.querySelector(".backdrop"),y=document.querySelector(".js-modal-container");document.querySelector(".close-btn");let D="64f389465ae26083f39b17a9",w="",a={},N=new M;async function B(e){return w=e,await(await N.getExerciseById(w)).json()}async function O(e){try{const t=await B(e),i=y.querySelector(".js-add-remove-btn");let n="",o="",s;t&&I(t,C())?s=`
    ...
    <div class="modal-ex-img-container"
      ...
      <div class="modal-ex-text-info">
        ...
        </li>`,"")}H.addEventListener("submit",e=>{e.preventDefault(),e.target.value.trim()});function P(e,t){return Array.from({length:e}).reduce((i,n,o)=>{const s=o+1;return i+`<li class="pagination-element ${s===t?"active_pag_item":""}" value="${s}">${s}</li>`},"")}const ce=document.querySelector(".search-container"),L=document.querySelector(".filters-box"),b=document.getElementById("cards-list"),le=L.querySelector(".filters-list-item"),de=le.dataset.filter,u=document.querySelector(".pagination-list"),ue=window.innerWidth;let l=null,me=1;ve(ue);let j=new M;const c={filter:de,page:me,limit:l};async function ge(){let e=await j.init();try{const i=await(await e.getListExercises(c)).json();if(i.results&&i.results.length>0){b.innerHTML=R(i.results),u.innerHTML="";let n=P(i.totalPages,c.page);u.innerHTML=n}else console.error("No exercises found.")}catch(t){console.error("Error fetching exercises:",t)}}function ve(e){return e<768?l=8:(e>=768&&e<1024,l=12),l}function fe(e){const t=parseInt(e.target.getAttribute("value"));c.page=t,console.log(t),_(e)}function pe(e){L.querySelectorAll(".filters-list-item").forEach(t=>{t.classList.remove("active_item")}),e.target.classList.add("active_item")}async function _(e){if(e.target.classList.contains("filters-list-item")){pe(e);const t=e.target.dataset.filter;c.filter=t;let i=await j.init();try{const o=await(await i.getListExercises(c)).json();if(o.results&&o.results.length>0){b.innerHTML=R(o.results),u.innerHTML="";let s=P(o.totalPages,c.page);u.innerHTML=s}else console.error("No exercises found.")}catch(n){console.error("Error fetching exercises:",n)}}}async function be(e){const t=e.target.dataset.name,i=e.target.dataset.filter;if(t&&i){let n=i;i==="body parts"&&(n="bodypart"),await re(n,t)}}function R(e){return e.reduce((t,i)=>t+`<li class="cards-list-item" data-filter="${i.filter.toLowerCase()}" data-name="${i.name}" style="background:linear-gradient(
      0deg,
      rgba(16, 16, 16, 0.7) 0%,
    ...
    </li>`,"")}document.addEventListener("DOMContentLoaded",ge);L.addEventListener("click",_);b.addEventListener("click",be);u.addEventListener("click",fe);const x=document.querySelector(".back-to-top");x.addEventListener("click",he);window.onscroll=()=>{ye()};function he(){window.scrollTo({top:0,behavior:"smooth"})}function ye(){document.body.scrollTop>20||document.documentElement.scrollTop>20?x.classList.remove("is-hidden"):x.classList.add("is-hidden")}const we=document.querySelector(".icon-about-sport");let q=0;function xe(){q+=1,we.style.transform=`rotate(${q}deg)`}setInterval(xe,10);
