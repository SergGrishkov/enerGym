import{E as M,f as A}from"./assets/quote-c390530b.js";import"./assets/vendor-0fffa566.js";const y=document.querySelector(".backdrop"),h=document.querySelector(".js-modal-container");document.querySelector(".close-btn");let D="64f389465ae26083f39b17a9",w="",a={},N=new M;async function B(e){return w=e,await(await N.getExerciseById(w)).json()}async function O(e){try{const t=await B(e),i=h.querySelector(".js-add-remove-btn");let n="",o="",s;t&&I(t,C())?s=`
    ...
    <div class="modal-ex-img-container"
      ...
      <div class="modal-ex-text-info">
        ...
        </li>`,"")}H.addEventListener("submit",e=>{e.preventDefault(),e.target.value.trim()});function P(e,t){return Array.from({length:e}).reduce((i,n,o)=>{const s=o+1;return i+`<li class="pagination-element ${s===t?"active_pag_item":""}" value="${s}">${s}</li>`},"")}const ce=document.querySelector(".home-filters"),le=document.querySelector(".search-container"),L=document.querySelector(".filters-box"),b=document.getElementById("cards-list"),de=L.querySelector(".filters-list-item"),ue=de.dataset.filter,u=document.querySelector(".pagination-list"),me=window.innerWidth;let l=null,ge=1;pe(me);function ve(){ce.scrollIntoView({behavior:"smooth",block:"start"})}let j=new M;const c={filter:ue,page:ge,limit:l};async function fe(){let e=await j.init();try{const i=await(await e.getListExercises(c)).json();if(i.results&&i.results.length>0){b.innerHTML=R(i.results),u.innerHTML="";let n=P(i.totalPages,c.page);u.innerHTML=n}else console.error("No exercises found.")}catch(t){console.error("Error fetching exercises:",t)}}function pe(e){return e<768?l=8:(e>=768&&e<1024,l=12),l}async function be(e){const t=parseInt(e.target.getAttribute("value"));c.page=t,await _(e),ve()}function ye(e){L.querySelectorAll(".filters-list-item").forEach(t=>{t.classList.remove("active_item")}),e.target.classList.add("active_item")}async function _(e,t){if(t==="filter"||e.target.classList.contains("filters-list-item")){ye(e);const n=e.target.dataset.filter;c.filter=n,c.page=1}let i=await j.init();try{const o=await(await i.getListExercises(c)).json();if(o.results&&o.results.length>0){b.innerHTML=R(o.results),u.innerHTML="";let s=P(o.totalPages,c.page);u.innerHTML=s,he()}else console.error("No exercises found.")}catch(n){console.error("Error fetching exercises:",n)}}function he(){const e=document.querySelectorAll(".cards-list-item");e.forEach(t=>{t.style.opacity="0",t.style.transition="opacity 0.5s ease-in"}),e.forEach((t,i)=>{setTimeout(()=>{t.style.opacity="1"},i*100)})}async function we(e){const t=e.target.dataset.name,i=e.target.dataset.filter;if(t&&i){let n=i;i==="body parts"&&(n="bodypart"),await re(n,t)}}function R(e){return e.reduce((t,i)=>t+`<li class="cards-list-item" data-filter="${i.filter.toLowerCase()}" data-name="${i.name}" style="background:linear-gradient(
      0deg,
      rgba(16, 16, 16, 0.7) 0%,
