import{i as M,r as W,a as O,E as w,f as A}from"./assets/quote-13dbbd1a.js";import"./assets/vendor-ebddff4e.js";const E=document.body,y=document.querySelector(".backdrop"),B=document.querySelector(".js-modal-container");document.querySelector(".close-btn");let c;function D(e){c=e,e._id,y.classList.add("is-open"),E.style.overflow="hidden",window.addEventListener("keydown",b),window.addEventListener("click",h);let n;M(e._id)?n=`
    ...
    <div class="modal-ex-img-container"
      ...
      <div class="modal-ex-text-info">
        ...
        </li>`,"")}function X(){return"<p>Unfortunately, <span>no results</span> were found. You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs.</p>"}p.addEventListener("submit",async e=>{e.preventDefault();const n={[r.filter]:r.name,page:1,limit:10},t=p.elements.delay.value.trim();n.keyword=t;const i=await(await V.getListExercisesBySubspecies(n)).json();t&&i.results.length>0?(m.innerHTML=C(i.results),p.reset()):m.innerHTML=X()});function R(e,n){return Array.from({length:e}).reduce((t,i,s)=>{const o=s+1;return t+`<li class="pagination-element ${o===n?"active_pag_item":""}" value="${o}">${o}</li>`},"")}const ee=document.querySelector(".home-filters"),I=document.querySelector(".search-container"),S=document.querySelector(".filters-box"),m=document.getElementById("cards-list"),te=S.querySelector(".filters-list-item"),ne=te.dataset.filter,f=document.querySelector(".pagination-list"),ie=window.innerWidth;let g=null,se=1;re(ie);function oe(){ee.scrollIntoView({behavior:"smooth",block:"start"})}let H=new w;const d={filter:ne,page:se,limit:g};async function ae(){let e=await H.init();try{const t=await(await e.getListExercises(d)).json();if(t.results&&t.results.length>0){m.innerHTML=F(t.results),f.innerHTML="";let i=R(t.totalPages,d.page);f.innerHTML=i}else console.error("No exercises found.")}catch(n){console.error("Error fetching exercises:",n)}}function re(e){return e<768?g=8:(e>=768&&e<1024,g=12),g}async function le(e){const n=parseInt(e.target.getAttribute("value"));d.page=n,await j(e),oe()}function ce(e){S.querySelectorAll(".filters-list-item").forEach(n=>{n.classList.remove("active_item")}),e.target.classList.add("active_item")}function de(){const e=document.querySelectorAll(".animated-card");e.forEach(n=>{n.style.opacity="0"}),e.forEach((n,t)=>{setTimeout(()=>{n.style.opacity="1"},t*100)})}async function j(e,n){if(n==="filter"||e.target.classList.contains("filters-list-item")){ce(e);const i=e.target.dataset.filter;d.filter=i,d.page=1}let t=await H.init();try{const s=await(await t.getListExercises(d)).json();if(s.results&&s.results.length>0){m.innerHTML=F(s.results),I.innerHTML="",f.innerHTML="";let o=R(s.totalPages,d.page);f.innerHTML=o,de()}else console.error("No exercises found.")}catch(i){console.error("Error fetching exercises:",i)}}async function ue(e){const n=e.target.dataset.name,t=e.target.dataset.filter;if(n&&t){let i=t;t==="body parts"&&(i="bodypart"),await Q(i,n)}}function F(e){return e.reduce((n,t)=>n+`<li class="cards-list-item animated-card" data-filter="${t.filter.toLowerCase()}" data-name="${t.name}" style="background:linear-gradient(
      0deg,
      rgba(16, 16, 16, 0.7) 0%,
    ...
    </li>`,"")}document.addEventListener("DOMContentLoaded",ae);S.addEventListener("click",e=>j(e,"filter"));m.addEventListener("click",ue);f.addEventListener("click",e=>le(e));const N=document.querySelector(".js-modal"),me=document.querySelector(".js-close"),ge=document.querySelector(".mrating"),pe=document.querySelector(".rating-form");document.querySelector(".js-sbutton");const fe=document.querySelector(".message");function ve(){N.classList.add("is-open")}ve();function _(){N.classList.remove("is-open")}me.addEventListener("click",_);let P,$;ye(ge);function ye(e){x(e),v(),e.classList.contains("rating-set")&&be(e)}function x(e){P=e.querySelector(".mrating-active"),$=e.querySelector(".mrating-value")}function v(e=$.innerHTML){const n=e/.05;P.style.width=`${n}%`}function be(e){const n=e.querySelectorAll(".mrating-item");for(let t=0;t<n.length;t+=1){const i=n[t];i.addEventListener("mouseenter",function(s){x(e),v(i.value)}),i.addEventListener("mouseleave",function(s){v()}),i.addEventListener("click",function(s){x(e);const o=t+1;$.innerHTML=o,v()})}}let he=new w;pe.addEventListener("click",we);async function we(e){if(e.preventDefault(),!e.target.tagName!=="BUTTON"&&e.target.tagName==="BUTTON"){if(+e.currentTarget.children[0].innerText.trim()<=0)return;const n=e.currentTarget.elements.ratbtn.dataset.exerciseId,t={};t.rate=+e.currentTarget.children[0].innerText.trim(),t.email=e.currentTarget.elements.email.value,t.review=e.currentTarget.elements.comment.value;try{const i=await he.addRating(n,t),s=i.info();s.status===200?(_(),D(i.json())):fe.textContent=s.message}catch(i){console.error(i)}}}const T=document.querySelector(".back-to-top");T.addEventListener("click",Le);window.onscroll=()=>{Ee()};function Le(){window.scrollTo({top:0,behavior:"smooth"})}function Ee(){document.body.scrollTop>20||document.documentElement.scrollTop>20?T.classList.remove("is-hidden"):T.classList.add("is-hidden")}const xe=document.querySelector(".icon-about-sport");let q=0;function Te(){q+=1,xe.style.transform=`rotate(${q}deg)`}setInterval(Te,10);
