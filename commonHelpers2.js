import{E as M,f as q}from"./assets/quote-4b8e4c4f.js";import"./assets/vendor-0fffa566.js";const h=document.querySelector(".backdrop"),y=document.querySelector(".js-modal-container");document.querySelector(".close-btn");let W="64f389465ae26083f39b17a9",w="",a={},D=new M;async function A(e){return w=e,await(await D.getExerciseById(w)).json()}async function N(e){try{const t=await A(e),i=y.querySelector(".js-add-remove-btn");let n="",s="",o;t&&C(t,B())?o=`
    ...
    <div class="modal-ex-img-container"
      ...
      <div class="modal-ex-text-info">
      ...
      </div>`}catch(t){console.error("Error fetching or creating markup:",t)}finally{let s=function(){o(),S()},o=function(){t=document.querySelector("#rating-active"),i=document.querySelector(".rating-value"),console.log("value",i.innerHTML)},S=function(){console.log(i.innerHTML);const R=i.innerHTML/.05;t.style.width=`${R}%`},t,i;document.querySelector(".rating")&&s()}}N(W);function m(e){e.key==="Escape"&&(h.classList.remove("is-open"),window.removeEventListener("keydown",m),window.removeEventListener("click",g))}function g(e){console.log(e.target.classList.value),(e.target.classList.value==="modal-close-icon"||e.target.classList.value==="backdrop is-open")&&(h.classList.remove("is-open"),window.removeEventListener("click",g),window.removeEventListener("keydown",m)),e.target.classList.value.includes("rating-btn")&&(h.classList.remove("is-open"),window.removeEventListener("click",g),window.removeEventListener("keydown",m))}window.addEventListener("keydown",m);window.addEventListener("click",g);function B(){const e=localStorage.getItem("favorite-exercise");return e?JSON.parse(e):[]}function $(e){e.length===0?localStorage.removeItem("favorite-exercise"):localStorage.setItem("favorite-exercise",JSON.stringify(e))}function C(e,t){return t.some(i=>i._id===e._id)}async function O(){try{a._id||(a=await A(w));const e=B();if(C(a,e)){const i=e.filter(n=>n._id!==a._id);$(i),T(!1),console.log("Exercise removed from favorites:",a)}else e.push(a),$(e),T(!0),console.log("Exercise added to favorites:",a)}catch(e){console.error("Error toggling favorites:",e)}}document.addEventListener("click",function(e){e.target.classList.contains("add-favorite")&&O()});function T(e){const t=e?"./sprite.svg#icon-remove-favorites":"./sprite.svg#icon-add-favorites",i=e?"Remove from favorites":"Add to favorites",n=y.querySelector(".js-add-remove-btn");console.log("isFavorite",e),n.innerHTML=`<button class="ex-modal-btn add-favorite" type="button">${i}<p class="btn-icon-add-remove-favorite js-add-remove-btn"><svg class="modal-icon-favorite" width="18" height="18" aria-label="modal favorite icon"><use href='${t}'></use></svg></p></button>`}let F,v;const U=document.querySelector(".rating");U&&J();function J(){V(),z()}function V(){F=document.querySelector("#rating-active"),v=document.querySelector(".rating-value"),console.log("value",v.innerHTML)}function z(){console.log(v.innerHTML);const e=v.innerHTML/.05;F.style.width=`${e}%`}const E=document.getElementById("confirmationModal");document.querySelector(".modal-subscribe-close-btn");document.querySelector(".modal-subscribe-close-button");const Y=document.querySelector(".footer-form"),d=document.getElementById("modalSubscribeText");let f=!1;function p(){E.classList.remove("is-open");const e="⭐You will receive notifications about new exercises!";d.textContent=e,f=!1}const G=document.querySelectorAll(".modal-subscribe-close-btn, .modal-subscribe-close-button");G.forEach(e=>{e.addEventListener("click",p)});document.addEventListener("keydown",function(e){f&&e.key==="Escape"&&p()});window.addEventListener("click",function(e){f&&e.target===E&&p()});function k(){E.classList.add("is-open"),f=!0,setTimeout(p,8e3)}function K(){if(d){const e="You are already subscribed";d.textContent=e}else console.error('Element with id "modalSubscribeText" not found.')}function Q(){if(d){const e="☝ But you entered a bad or invalid email ✉.  Please try another one! ";d.textContent=e}else console.error('Element with id "modalSubscribeText" not found.')}async function X(e){e.preventDefault();const t=document.querySelector('[name="footer-email"]'),i=t.value.trim();try{const n=await fetch("https://energyflow.b.goit.study/api/subscription",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:i})});if(t.value="",n.ok)k();else{const s=n.status===400?"☝ But you entered a bad or invalid email ✉. Please try another one!":`An error occurred when sending a request to the server: ${n.statusText}`;console.error(s),n.status===400?Q(s):K(s),k()}}catch(n){console.error(`An error occurred while executing the request: ${n}`),alert("An error occurred while executing the request")}}Y.addEventListener("submit",X);const H=document.querySelector(".form");document.querySelector(".modal-exercise");const Z=window.innerWidth;let ee,te,ie=1,ne;const r={filter:ee,name:te,page:ie,limit:ne};function se(e){return e<1280?r.limit=8:r.limit=9,r.limit}se(Z);async function oe(e,t){r.filter=e,r.name=t;const i=`https://energyflow.b.goit.study/api/exercises?${r.filter}=${r.name}&page=${r.page}&limit=${r.limit}`;try{const s=await(await fetch(i)).json();if(s.results){const o=s.results;b.innerHTML=re(o),ae.insertAdjacentElement("beforeEnd",H)}}catch(n){console.log(n)}}function re(e){return e.reduce((t,i)=>t+`<li class="list-item" data-exerciseid="${i._id}">
        ...
        </li>`,"")}H.addEventListener("submit",e=>{e.preventDefault(),e.target.value.trim()});function I(e,t){return Array.from({length:e}).reduce((i,n,s)=>{const o=s+1;return i+`<li class="pagination-element ${o===t?"active_pag_item":""}" value="${o}">${o}</li>`},"")}const ae=document.querySelector(".search-container"),L=document.querySelector(".filters-box"),b=document.getElementById("cards-list"),ce=L.querySelector(".filters-list-item"),le=ce.dataset.filter,u=document.querySelector(".pagination-list"),de=window.innerWidth;let l=null,ue=1;ge(de);let P=new M;const c={filter:le,page:ue,limit:l};async function me(){let e=await P.init();try{const i=await(await e.getListExercises(c)).json();if(i.results&&i.results.length>0){b.innerHTML=_(i.results),u.innerHTML="";let n=I(i.totalPages,c.page);u.innerHTML=n}else console.error("No exercises found.")}catch(t){console.error("Error fetching exercises:",t)}}function ge(e){return e<768?l=8:(e>=768&&e<1024,l=12),l}function ve(e){const t=parseInt(e.target.getAttribute("value"));c.page=t,console.log(t),j(e)}function fe(e){L.querySelectorAll(".filters-list-item").forEach(t=>{t.classList.remove("active_item")}),e.target.classList.add("active_item")}async function j(e){if(e.target.classList.contains("filters-list-item")){fe(e);const t=e.target.dataset.filter;c.filter=t;let i=await P.init();try{const s=await(await i.getListExercises(c)).json();if(s.results&&s.results.length>0){b.innerHTML=_(s.results),u.innerHTML="";let o=I(s.totalPages,c.page);u.innerHTML=o}else console.error("No exercises found.")}catch(n){console.error("Error fetching exercises:",n)}}}async function pe(e){const t=e.target.dataset.name,i=e.target.dataset.filter;if(t&&i){let n=i;i==="body parts"&&(n="bodypart"),await oe(n,t)}}function _(e){return e.reduce((t,i)=>t+`<li class="cards-list-item" data-filter="${i.filter.toLowerCase()}" data-name="${i.name}" style="background:linear-gradient(
      0deg,
      rgba(16, 16, 16, 0.7) 0%,
    ...
    </li>`,"")}document.addEventListener("DOMContentLoaded",me);L.addEventListener("click",j);b.addEventListener("click",pe);u.addEventListener("click",ve);const x=document.querySelector(".back-to-top");x.addEventListener("click",be);window.onscroll=()=>{he()};function be(){window.scrollTo({top:0,behavior:"smooth"})}function he(){document.body.scrollTop>20||document.documentElement.scrollTop>20?x.classList.remove("is-hidden"):x.classList.add("is-hidden")}
